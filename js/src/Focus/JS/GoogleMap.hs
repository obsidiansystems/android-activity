...
geolocating :: (MonadWidget t m, HasJS x (WidgetHost m))
...
googleMap :: forall t m k x. (HasJS x m, HasJS x (WidgetHost m), MonadWidget t m, Ord k)
...
searchInput :: forall t m a k. (DomBuilder t m, PostBuild t m, MonadHold t m, MonadFix m, Ord k) => Dynamic t (Map Text Text) -> Event t (Map k (Text, a)) -> m (Event t Text, Event t (Text, a), Dynamic t Text)
searchInput attrs results = searchInput' "" never attrs results searchInputResultsList

searchInput' :: forall t m a k. (DomBuilder t m, PostBuild t m, MonadHold t m, MonadFix m, Ord k) => Text -> Event t Text -> Dynamic t (Map Text Text) -> Event t (Map k (Text, a)) -> (Dynamic t (Map k (Text, a)) -> m (Event t (Text, a))) -> m (Event t Text, Event t (Text, a), Dynamic t Text)
searchInput' v0 setV attrs results listBuilder = do
  ...
  rec input <- inputElement $ def
      ...
      let eMadeChoiceViaEnter = fmapMaybe (fmap fst . Map.minView) $ tag (current dResults) enter
          ...
          eInputChanged = fmapMaybe id $ leftmost [eSelectionMade, fmap Just (_inputElement_input input), fmap Just (tag (current $ value input) enter)]
...
twoSourceSearch :: forall t x m a b. (HasJS x m, MonadWidget t m)
                => Text
                -> Text
                -> Text
                ...
                case n of
                     ...
                     False -> do
                       ...
                       addressComponents <- liftM toAddressComponents $ forM acRef $ \ac -> do
                         ...
                         return (T.pack longName, T.pack shortName, map T.pack types)
...
googleMapsAutocompletePlace' s cb =  do
  rec jsCb <- mkJSFun $ \(result:_) -> do
        ...
        a' <- forM a $ \p -> do
                ...
                return (T.pack desc, PlacesAutocompletePredictionReference ref)
        liftIO $ cb a'
        freeJSFun jsCb
        mkJSUndefined
...
googleMapsAutocompletePlace :: forall x m t. (HasJS x (WidgetHost m), MonadWidget t m)
...
geocodeSearch :: forall t m x. (HasJS x m, HasJS x (WidgetHost m), DomBuilder t m, PostBuild t m, PerformEvent t m, TriggerEvent t m, MonadHold t m, MonadFix m)
              => Text
              -- ^ Path to Google logo
              -> Text
